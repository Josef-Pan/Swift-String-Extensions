...
extension String{
    ...
    subscript (r: ClosedRange<Int>) -> String{ // ✅  Safe to use Range from any indices, even exceeding boundary
        get {
            let lowerBound  = min(max(r.lowerBound, 0), self.count-1) // Keep starting idx between 0 and boundary
            let upperBound  = max(min(r.upperBound, self.count-1), 0) // Keep ending idx between 0 and boundary
            let startIndex = self.index(self.startIndex, offsetBy: lowerBound)
    ...
    subscript (r: PartialRangeUpTo<Int>) -> String{ // ✅  Safe to use Range from any indices, even exceeding boundary
        get {
    ...
    subscript (r: PartialRangeThrough<Int>) -> String{ // ✅ Safe to use Range from any indices, even exceeding boundary
        get {
            let upperBound  = max(min(r.upperBound, self.count-1), 0) // Keep ending idx between 0 and boundary
            let endIndex = self.index(self.startIndex, offsetBy: upperBound)
            return String(self[startIndex...endIndex])
    ...
    subscript (r: PartialRangeFrom<Int>) -> String{ // ✅ Safe to use Range from any indices, even exceeding boundary
        get {
            let lowerBound  = min(max(r.lowerBound, 0), self.count-1) // Keep starting idx between 0 and boundary
            let startIndex = self.index(self.startIndex, offsetBy: lowerBound)
